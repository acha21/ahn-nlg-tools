...
third_party_path = os.path.join(NLG_TOOLS_DIR, "data/eval/")
...
def calc_nist_bleu(path_refs, path_hyp, fld_out="temp", n_lines=None):
    ...
    if n_lines is None:
        with open(path_hyp, encoding="utf-8") as fr:
    ...
    cmd = [
        "perl",
        third_party_path + "3rdparty/mteval-v14c.pl",
        "-s",
        "%s/src.xml" % fld_out,
        "-t",
        "%s/hyp.xml" % fld_out,
...
def calc_cum_bleu(path_refs, path_hyp):
    ...
    process = subprocess.Popen(
        ["perl", third_party_path + "3rdparty/multi-bleu.perl"] + path_refs,
...
def calc_meteor(path_refs, path_hyp, fld_out="temp", n_lines=None, pretokenized=True):
    ...
    cmd = [
        ...
        "-jar",
        third_party_path + "3rdparty/meteor-1.5/meteor-1.5.jar",
        path_hyp,
        path_merged_refs,
        "-r",
...
def _write_xml(paths_in, path_out, role, n_lines=None):
    ...
    for i_in, path_in in enumerate(paths_in):
        ...
        if role == "src":
            ...
            lines.append(
                '<refset setid="unnamed" srclang="src" trglang="tgt" refid="ref%i">'
                % i_in
            )
            set_ending = "</refset>"
